...
const EXTENSION_VERSION = "3.0.0-dev3";
...
runButton.addEventListener("click", async () => {
  ...
  result.innerHTML = "Running QC DEV3... Please wait.";
  ...
  try {
    ...
    if (!tab?.id) {
      result.innerHTML = "Active admin tab not found.";
    ...
    chrome.tabs.sendMessage(
      ...
      async response => {
        ...
        if (chrome.runtime.lastError || !response) {
          result.innerHTML = "Admin page refresh karo, Preview Test & Questions open karo, phir Run QC DEV3 dabao.";
          return;
        }
        ...
        if (excelFile) {
          try {
            await applyExcelCompare(response, excelFile);
          } catch {
            addReportRow(response, {
              priority: "Critical",
              type: "Test",
              ...
              check: "Excel File",
              status: "ERROR",
              details: "Excel file could not be read"
            });
          }
        }
        ...
        if (pdfFile) {
          try {
            await applyPdfCompare(response, pdfFile);
          } catch {
            addReportRow(response, {
              priority: "Critical",
              type: "Test",
              ...
              check: "PDF File",
              status: "ERROR",
              details: "PDF file could not be read"
            });
          }
        }
